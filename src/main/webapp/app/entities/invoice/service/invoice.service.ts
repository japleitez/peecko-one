...
type RestOf<T extends IInvoice | NewInvoice> = Omit<T, 'issued' | 'dueDate' | 'saleDate' | 'paidDate'> & {
  issued?: string | null;
  dueDate?: string | null;
...
export class InvoiceService {
  ...
  search(req?: any): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http
      .get<RestInvoice[]>(`${this.resourceUrl}/search`, { params: options, observe: 'response' })
      .pipe(map(res => this.convertResponseArrayFromServer(res)));
  }
  ...
  protected convertDateFromClient<T extends IInvoice | NewInvoice | PartialUpdateInvoice>(invoice: T): RestOf<T> {
    return {
      ...invoice,
      issued: invoice.issued?.toJSON() ?? null,
      dueDate: invoice.dueDate?.format(DATE_FORMAT) ?? null,
      saleDate: invoice.saleDate?.format(DATE_FORMAT) ?? null,
      paidDate: invoice.paidDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restInvoice: RestInvoice): IInvoice {
    return {
      ...restInvoice,
      issued: restInvoice.issued ? dayjs(restInvoice.issued) : undefined,
      dueDate: restInvoice.dueDate ? dayjs(restInvoice.dueDate) : undefined,
      saleDate: restInvoice.saleDate ? dayjs(restInvoice.saleDate) : undefined,
      paidDate: restInvoice.paidDate ? dayjs(restInvoice.paidDate) : undefined,
    };
  }
