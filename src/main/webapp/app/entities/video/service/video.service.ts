...
type RestOf<T extends IVideo | NewVideo> = Omit<T, 'created' | 'released' | 'archived'> & {
  created?: string | null;
...
export class VideoService {
  ...
  protected convertDateFromClient<T extends IVideo | NewVideo | PartialUpdateVideo>(video: T): RestOf<T> {
    return {
      ...video,
      created: video.created?.toJSON() ?? null,
      released: video.released?.toJSON() ?? null,
      archived: video.archived?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restVideo: RestVideo): IVideo {
    return {
      ...restVideo,
      created: restVideo.created ? dayjs(restVideo.created) : undefined,
      released: restVideo.released ? dayjs(restVideo.released) : undefined,
      archived: restVideo.archived ? dayjs(restVideo.archived) : undefined,
    };
  }
