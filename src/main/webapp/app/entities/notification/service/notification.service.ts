...
type RestOf<T extends INotification | NewNotification> = Omit<T, 'starts' | 'expires'> & {
  starts?: string | null;
  expires?: string | null;
};
...
export class NotificationService {
  ...
  protected convertDateFromClient<T extends INotification | NewNotification | PartialUpdateNotification>(notification: T): RestOf<T> {
    return {
      ...notification,
      starts: notification.starts?.format(DATE_FORMAT) ?? null,
      expires: notification.expires?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restNotification: RestNotification): INotification {
    return {
      ...restNotification,
      starts: restNotification.starts ? dayjs(restNotification.starts) : undefined,
      expires: restNotification.expires ? dayjs(restNotification.expires) : undefined,
    };
  }
