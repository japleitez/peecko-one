...
public interface ApsOrderRepository extends JpaRepository<ApsOrder, Long>, JpaSpecificationExecutor<ApsOrder> {
    ...
    @Query("from ApsOrder o left join fetch o.apsPlan where o.agencyId = :agencyId and o.period = :period")
    List<ApsOrder> getByAgencyAndPeriod(@Param("agencyId") Long agencyId, @Param("period") Integer period);

    @Query("from ApsOrder o left join fetch o.apsPlan where o.apsPlan.contract = :contract and o.period = :period")
    List<ApsOrder> getByContactAndPeriod(@Param("contract") String contract, @Param("period") Integer period);


    @Query("from ApsOrder o left join fetch o.apsPlan where o.agencyId = :agencyId and o.period = :period and o.apsPlan.state = 'ACTIVE'")
    List<ApsOrder> getByAgencyAndPeriodAndActive(@Param("agencyId") Long agencyId, @Param("period") Integer period);

    @Query("from ApsOrder o left join fetch o.apsPlan where o.apsPlan.contract = :contract and o.period = :period and o.apsPlan.state = 'ACTIVE'")
    List<ApsOrder> getByContractAndPeriodAndActive(@Param("contract") String contract, @Param("period") Integer period);

    @Query("from ApsOrder o left join fetch o.apsPlan where o.customerId = :customerId and o.period between :startPeriod and :endPeriod order by o.period")
    ...
    @Query("from ApsOrder o left join fetch o.apsPlan where o.customerId = :customerId and o.period >= :startPeriod order by o.period")
    List<ApsOrder> getByCustomerAndStartPeriod(@Param("customerId") Long customerId, @Param("startPeriod") Integer startPeriod);

    @Query("from ApsOrder o left join fetch o.apsPlan where o.customerId = :customerId and o.period <= :endPeriod order by o.period")
