...
export class VideoCategoryComponent implements OnInit {
  ...
  trackId = (_index: number, item: IVideoCategory): number => this.videoCategoryService.getVideoCategoryIdentifier(item);
  ...
  protected refineData(data: IVideoCategory[]): IVideoCategory[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IVideoCategory[] | null): IVideoCategory[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.videoCategoryService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
