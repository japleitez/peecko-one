...
type RestOf<T extends IArticle | NewArticle> = Omit<T, 'created' | 'updated' | 'released' | 'archived'> & {
  created?: string | null;
  updated?: string | null;
...
export class ArticleService {
  ...
  protected convertDateFromClient<T extends IArticle | NewArticle | PartialUpdateArticle>(article: T): RestOf<T> {
    return {
      ...article,
      created: article.created?.toJSON() ?? null,
      updated: article.updated?.toJSON() ?? null,
      released: article.released?.toJSON() ?? null,
      archived: article.archived?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restArticle: RestArticle): IArticle {
    return {
      ...restArticle,
      created: restArticle.created ? dayjs(restArticle.created) : undefined,
      updated: restArticle.updated ? dayjs(restArticle.updated) : undefined,
      released: restArticle.released ? dayjs(restArticle.released) : undefined,
      archived: restArticle.archived ? dayjs(restArticle.archived) : undefined,
    };
  }
